...
def get_model_index(total_indices, A_idx, *, time_thres_idx, image_width):
    ...
    temp_index = total_indices[A_idx][1]
    model_index = [
        list(range(k - image_width, k + image_width + 1))
        for k in temp_index
        if (((k - image_width) > 0) & (k < (time_thres_idx - image_width)))
    ]

    if not model_index:
        raise ValueError(f"[get_model_index] No valid index found for A_idx {A_idx}")

    return np.array(model_index)
