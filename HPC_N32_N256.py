...
total_indices = np.load('./data/total_indices/total_indices_v4_N{}.npy'.format(N_PULSE), allow_pickle=True).item()

exp_data = np.load('./data/exp_data/exp_data_{}.npy'.format(N_PULSE)).flatten()  # the experimental data to be evalutated
exp_data_deno = np.load('./data/exp_data/exp_data_{}_deno.npy'.format(N_PULSE))  # the denoised experimental data to be evalutated
time_data = np.load('./data/time_data/time_data_{}.npy'.format(N_PULSE))          # the time data for the experimental data to be evalutated
spin_bath = np.load('./data/spin_bath/spin_bath_M_value_N{}.npy'.format(N_PULSE))
# if N_PULSE == 32:
#     spin_bath = np.load('./data/spin_bath/spin_bath_M_value_N{}.npy'.format(N_PULSE)) # the spin bath data for the experimental N_PULSE (it is not pre-requisite so one can just ignore this line.)
# elif N_PULSE == 256:
#     spin_bath = np.load('./data/spin_bath/spin_bath_M_value_N{}_exp256_timerange.npy'.format(N_PULSE)) # the spin bath data for the experimental N_PULSE (it is not pre-requisite so one can just ignore this line.)
