...
X_train_arr = np.load('data/noisy_px/X_train.npy') # recommenended total batch size is over than 500,000 samples
Y_train_arr = np.load('data/noisy_px/Y_train.npy')
X_valid_arr = np.load('data/noisy_px/X_valid.npy')
Y_valid_arr = np.load('data/noisy_px/Y_valid.npy')
...
for epoch in range(epochs):
    ...
    for i in range(train_batch // mini_batch):
        ...
        y_train_temp = torch.Tensor(Y_train_arr[train_indices]).cuda().squeeze(2)
    ...
    with torch.no_grad():
        ...
        Y_valid = torch.Tensor(Y_valid_arr[valid_indices]).cuda().squeeze(2)
...
# with torch.no_grad():
#     denoised_train = model(torch.Tensor(X_train_arr).cuda()).cpu().numpy()
#     np.save('./data/noisy_px/denoised_train.npy', denoised_train)
#
#     denoised_valid = model(torch.Tensor(X_valid_arr).cuda()).cpu().numpy()
#     np.save('data/noisy_px/denoised_valid.npy', denoised_valid)
